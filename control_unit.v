...
always @(*) begin
    if(state == IDLE) next_state = FETCH;
    else if(state == FETCH) next_state = DECODE;
    ...
    else if(state == WB) next_state = FETCH;          // always go fetch next
    else if(state == HALT) next_state = HALT;           // stay here forever
    else next_state = IDLE;           // safety default
end
